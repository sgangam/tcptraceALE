	...
	Bool * pout_order)
{
    ...
    if (thisseg_firstbyte >= 3329165351 && thisseg_firstbyte <= 3329317335)
    {}
		 ...
		 double etime_rtt)
{
    ...
    */
    ...
    if (&(ptcb->ptp->a2b) == ptcb) {
        sIp = ptcb->ptp->addr_pair.a_address ;
        dIp = ptcb->ptp->addr_pair.b_address ;
        sPort = ptcb->ptp->addr_pair.a_port ;
        dPort = ptcb->ptp->addr_pair.b_port ;
    }
    else {
        sIp = ptcb->ptp->addr_pair.b_address ;
        dIp = ptcb->ptp->addr_pair.a_address ;
        sPort = ptcb->ptp->addr_pair.b_port ;
        dPort = ptcb->ptp->addr_pair.a_port ;
    }
    char sIpStr[INET_ADDRSTRLEN] ;
    char dIpStr[INET_ADDRSTRLEN] ;
    inet_ntop(AF_INET,& (sIp.un.ip4), sIpStr, INET_ADDRSTRLEN) ;
    inet_ntop(AF_INET,& (dIp.un.ip4), dIpStr, INET_ADDRSTRLEN) ;

    //Mfprintf (ptcb->rtt_dump_file, "%f %s %s %d %d %lu %f\n",
    fprintf (stdout, "%f %s %s %d %d %lu %f\n",
	      current_time.tv_sec + (current_time.tv_usec/1000000.0), sIpStr, dIpStr, sPort, dPort, pseg->seq_lastbyte + 1,
	      (float)(etime_rtt / 1000.0) /* convert from us to ms */ );
}
